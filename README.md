...
```text
 ...
 └────────┴────────┴────────┴────────┴────────┘      └────────┴────────┴────────┴────────┴────────┘
   ...
   terminal, where paragraphs do not exist. `del word` walks ⌥→ then ⌥⌫ — the
   whole word from anywhere inside it — and reaches the shell through four
   `bindkey` lines at the end of the dotfiles zshrc (Ghostty speaks CSI where
   kitty spoke ESC f/b; zsh only knew the latter). The ring toggles the firmware visual
