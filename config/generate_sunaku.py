...
def generate_macros():
    ...
    for f in fingers:
        ...
        # We need to release ALL specified modifiers, then tap the original finger's key, then tap the interrupter
        out += f"        sunaku_{f['name']}_tap: sunaku_{f['name']}_tap {{\\n"
        out += f"            compatible = \\\"zmk,behavior-macro-one-param\\\";\\n"
        out += f"            wait-ms = <0>;\\n            tap-ms = <0>;\\n            #binding-cells = <1>;\\n"
        
        # Create a release string for all modifiers we care about across all fingers so we never get stuck
        mod_releases = ""
...
def generate_layers():
    ...
    for f in fingers:
        ...
        # We need 76 keys for the Adv360.
        # Opposite hand: &trans (let the modifier apply)
        # Same hand: &sunaku_X_tap (cancel modifier and type both)
        # Thumbs: &trans (usually layer access keys, don't cancel)
        
        # Adv360 layout array
        bindings = ["&trans"] * 76
        
        # The keymap indices for left and right fingers
        left_alpha_indices = [
            1,2,3,4,5,
            15,16,17,18,19,
            29,30,31,32,33,
        ...
        # Adv360 keymap array maps to QWERTY/Dvorak
        # We need the physical key codes for these indices to pass to the macro parameter.
        # It's actually easier to just use the base QWERTY characters for these spots as the parameter.
        
        # We will map the specific alpha index to the keycode it represents in your Base layer
        # so the macro knows what to tap.
        keymap_mapping = {
            1: "N1", 2: "N2", 3: "N3", 4: "N4", 5: "N5",
            15: "Q", 16: "P", 17: "U", 18: "Y", 19: "SEMI",
            29: "A", 30: "O", 31: "E", 32: "I", 33: "G",
            47: "X", 48: "J", 49: "COMMA", 50: "EQUAL", 51: "SINGLE_QUOTE",
            67: "Z", 68: "C", 69: "V", # Using C/V mapped to your mod-morphs LT_LAUNCHER/PASTE roughly, but standard kp Z C V is fine for unrolling
...
if __name__ == "__main__":
    with open("config/sunaku_hrm.dtsi", "w") as f:
        f.write(header)
        f.write(generate_macros())
        f.write(generate_behaviors())
        f.write(generate_layers())
    print("Generated config/sunaku_hrm.dtsi successfully!")
